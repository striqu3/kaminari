...
const FOTOS_INICIAIS = 4;

export function GaleriaLightbox() {
  const [mostrarTodas, setMostrarTodas] = useState(false);
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

  const fotosExtras = GALERIA_FOTOS.slice(FOTOS_INICIAIS);
  ...
  return (
    <>
      {/* Grid 2x2 compacto - 4 fotos mesma proporção */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {GALERIA_FOTOS.slice(0, FOTOS_INICIAIS).map((foto, index) => (
          <div
            key={foto.src}
        ...
        <div className="mt-5 text-center">
          ...
          >
            {mostrarTodas ? (
              ...
              <><ChevronDown size={18} /> Ver mais {fotosExtras.length} fotos</>
            )}
          </button>
        </div>
      )}

      {/* Grid masonry para fotos extras */}
      {mostrarTodas && (
        <div className="columns-2 md:columns-3 lg:columns-4 gap-2 space-y-2 mt-3">
          {fotosExtras.map((foto, i) => (
            <div
              key={foto.src}
              className="break-inside-avoid cursor-pointer overflow-hidden rounded-lg group relative"
              onClick={() => abrirFoto(FOTOS_INICIAIS + i)}
            >
              <img
                src={foto.src}
                alt={foto.alt}
